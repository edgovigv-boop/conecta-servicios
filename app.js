...
const PWA_VERSION = "v4.9.51-dola-externo-copiar-pegar";
...
function mapPublication(row) {
  return {
    ...
    mediaUrl: row.image_url || row.multimedia_url || row.imagen_url || extractMediaUrlFromText(row.descripcion || "") || (isLikelyMediaUrl(row.ruta) ? row.ruta : ""),
...
function applyHomeFeedTemplateToWizard(template) {
  if (!template) return;
  const safeTemplate = typeof template === "object" ? template : {};
  const titleEl = document.getElementById("pubTitle");
  const descEl = activeDescriptionElement?.();
  const titleHint = document.getElementById("titleHint");
  if (titleEl && safeTemplate.titulo) titleEl.value = safeTemplate.titulo;
  if (descEl) {
    const text = safeTemplate.plantilla || safeTemplate.descripcion || "";
    if (text) {
      descEl.value = text;
      descEl.dispatchEvent(new Event("input", { bubbles: true }));
    }
  }
  if (titleHint) titleHint.textContent = "Usa este ejemplo como plantilla. Ajusta texto, zona, datos y contacto antes de publicar.";
}

function openOpportunityWizard(type, template = null) {
  resetWizardForm();
  const titleHint = document.getElementById("titleHint");
  const routeConfig = {
    ingresos: {
...
function isMissingMediaColumnsErrorV4931(error) {
  const text = String(error?.message || error || "").toLowerCase();
  return text.includes("image_url") || text.includes("media_type") || text.includes("media_path") || text.includes("media_created_at") || text.includes("schema cache");
}
function stripMediaColumnsForFallbackV4931(payload = {}) {
  const clean = { ...payload };
  delete clean.image_url;
  delete clean.media_type;
  delete clean.media_path;
  delete clean.media_created_at;
  return clean;
}
async function insertPublicationWithMediaFallbackV4931(payload) {
  try {
    return await supabaseRequest("publicaciones", { method: "POST", body: JSON.stringify(payload) });
  } catch (error) {
    if (!isMissingMediaColumnsErrorV4931(error)) throw error;
    console.warn("Columnas multimedia no disponibles en publicaciones; guardando URL dentro de descripción.", error.message);
    return await supabaseRequest("publicaciones", { method: "POST", body: JSON.stringify(stripMediaColumnsForFallbackV4931(payload)) });
  }
}
async function patchPublicationWithMediaFallbackV4931(id, payload) {
  try {
    return await supabaseRequest(`publicaciones?id=eq.${id}`, { method: "PATCH", body: JSON.stringify(payload) });
  } catch (error) {
    if (!isMissingMediaColumnsErrorV4931(error)) throw error;
    console.warn("Columnas multimedia no disponibles en publicaciones; guardando URL dentro de descripción.", error.message);
    return await supabaseRequest(`publicaciones?id=eq.${id}`, { method: "PATCH", body: JSON.stringify(stripMediaColumnsForFallbackV4931(payload)) });
  }
}
...
async function submitPublication(event) {
  ...
  const payload = formPayload();
  try {
    const [created] = await insertPublicationWithMediaFallbackV4931(payload);
  ...
  return `<article class="real-feed-card" id="pub-${item.id}">
    ...
    <div class="real-feed-body">
      ...
      <div class="real-feed-actions">
        ...
        ${typeof publicationAssistantButtonV4939 === "function" ? publicationAssistantButtonV4939(item) : ""}
...
function myPublicationCard(item) {
  ...
  return `<article class="compact-publication my-publication-card" id="my-pub-${item.id}">
    ...
    <div class="compact-detail">
      ...
      <div class="detail-actions">
        <button class="btn-small btn-purple" onclick="configurePublicationAssistantForItemV4939(${JSON.stringify(item).replace(/"/g, '&quot;')})">Configurar asistente</button>
        <button class="btn-small btn-outline" onclick="sharePublication('${item.id}')">Compartir</button>
...
function renderPilotFeedAdminEditor() {
  ...
  const rows = HOME_FEED_POSTS_V4918.map(post => {
    const ov = overrides[post.id] || {};
    const currentImg = displayMediaUrlV4931(ov.imagen || post.imagen || "", STORAGE_TEMPLATE_BUCKET_V4930);
    ...
    return `<details class="pilot-admin-item"><summary><span>${escapeHtml(homeFeedLabelV4918(post.tipo))}</span><strong>${escapeHtml(post.titulo)}</strong></summary>
      <div class="pilot-admin-row">
        <img src="${escapeHtml(currentImg)}" alt="${escapeHtml(post.titulo)}" onerror="this.onerror=null;this.src='assets/photo4922-servicios-1.webp';" />
        <div>
...
async function saveEdit(event) {
  ...
  try {
    await patchPublicationWithMediaFallbackV4931(id, payload);
    closeEditDialog();
...
const HOME_FEED_POSTS_V4918 = [
  {
    ...
    "descripcion": "Necesito apoyo para atender barra de café en un evento pequeño. Pago por día y busco alguien responsable.",
    "hashtags": [
      "Solicitante",
      "Trabajo",
      "Evento"
    ...
    "comentarios": 5,
    "compartidos": 12,
    "plantilla": "Necesito apoyo para atender barra de café en un evento pequeño. Busco una persona responsable, puntual y con buen trato. La idea es acordar horario, pago y detalles por contacto directo."
  },
  {
    "id": "v4921-solicitante-medicamento",
    "tipo": "solicitantes",
    "nombre": "Doña Carmen",
    "rol": "Solicitante",
    "ubicacion": "Chapultepec, México",
    "titulo": "Necesito que compren medicamento",
    "descripcion": "Busco alguien de confianza que pueda ir a la farmacia y entregar en mi domicilio.",
    "hashtags": [
      "Mandado",
      "Farmacia",
      "Ayuda"
    ...
    "comentarios": 8,
    "compartidos": 16,
    "plantilla": "Necesito que alguien me apoye comprando medicamento en farmacia y lo entregue en mi domicilio. Quiero acordar precio, horario y evidencia de compra antes de confirmar."
  },
  {
    "id": "v4921-solicitante-paquete",
    "tipo": "solicitantes",
    "nombre": "Javier Ruiz",
    "rol": "Solicitante",
    "ubicacion": "Toluca, México",
    "titulo": "Necesito enviar un paquete pequeño",
    "descripcion": "Busco quien lleve un paquete ligero de Toluca a Metepec hoy por la tarde.",
    "hashtags": [
      "Envío",
      "Paquete",
      "Local"
    ...
    "comentarios": 4,
    "compartidos": 10,
    "plantilla": "Necesito enviar un paquete pequeño dentro de la zona. Busco una persona disponible, con trato directo y confirmación de entrega."
  },
  {
    "id": "v4921-solicitante-viaje",
    "tipo": "solicitantes",
    "nombre": "Patricia Vega",
    "rol": "Solicitante",
    "ubicacion": "Metepec, México",
    "titulo": "Busco viaje compartido mañana",
    "descripcion": "Necesito traslado temprano hacia Toluca. Puedo cooperar con gasolina o cuota acordada.",
    "hashtags": [
      "Viaje",
      "Movilidad",
      "Compartido"
    ...
    "comentarios": 6,
    "compartidos": 11,
    "plantilla": "Busco viaje compartido hacia mi destino. Me interesa acordar punto de salida, horario, cooperación y trato seguro con la persona que viaje por la misma ruta."
  },
  {
    "id": "v4921-solicitante-muebles",
    "tipo": "solicitantes",
    "nombre": "Rafael Soto",
    "rol": "Solicitante",
    "ubicacion": "Metepec, México",
    "titulo": "Busco apoyo para mover muebles",
    "descripcion": "Necesito ayuda para mover dos muebles chicos dentro de la misma colonia.",
    "hashtags": [
      "Apoyo",
      "Mudanza",
      "Vecinos"
    ...
    "comentarios": 3,
    "compartidos": 8,
    "plantilla": "Necesito apoyo para mover algunos muebles dentro de mi zona. Busco acordar horario, costo y condiciones directamente con alguien disponible."
  },
  {
    "id": "v4921-agente-entregas",
    "tipo": "agentes",
    "nombre": "Carlos Méndez",
    "rol": "Agente",
    "ubicacion": "Guadalajara, Jalisco",
    "titulo": "Hago entregas locales por la tarde",
    "descripcion": "Tengo moto y puedo apoyar con paquetes pequeños, documentos y mandados cercanos.",
    "hashtags": [
      "Agente",
      "Entregas",
      "Moto"
    ...
    "comentarios": 8,
    "compartidos": 15,
    "plantilla": "Ofrezco apoyo para entregas locales por la tarde. Puedo llevar paquetes pequeños, documentos o pedidos en zonas cercanas con trato directo."
  },
  {
    "id": "v4921-agente-mandados",
    "tipo": "agentes",
    "nombre": "Pedro Luna",
    "rol": "Agente",
    "ubicacion": "Mexicaltzingo, México",
    "titulo": "Puedo hacer mandados verificados",
    "descripcion": "Realizo compras con evidencia de precio, ticket, cambio y entrega final.",
    "hashtags": [
      "Agente",
      "Mandados",
      "Evidencia"
    ...
    "comentarios": 4,
    "compartidos": 9,
    "plantilla": "Puedo realizar mandados verificados en mi zona. Registro evidencia del precio, compra, ticket, cambio y entrega para reducir dudas."
  },
  {
    "id": "v4921-agente-redes",
    "tipo": "agentes",
    "nombre": "Ana Torres",
    "rol": "Agente",
    "ubicacion": "Toluca, México",
    "titulo": "Apoyo negocios en redes sociales",
    "descripcion": "Puedo crear publicaciones, estados de WhatsApp y contenido básico por comisión.",
    "hashtags": [
      "Agente",
      "Redes",
      "Comisión"
    ...
    "comentarios": 10,
    "compartidos": 22,
    "plantilla": "Puedo ayudar a negocios locales con publicaciones, estados de WhatsApp y promoción sencilla. Me interesa trabajar por comisión o resultado acordado."
  },
  {
    "id": "v4921-agente-auto",
    "tipo": "agentes",
    "nombre": "Miguel Ángel",
    "rol": "Agente",
    "ubicacion": "Atizapán, México",
    "titulo": "Tengo auto y apoyo con traslados",
    "descripcion": "Puedo mover compras, paquetes medianos o acompañar entregas locales.",
    "hashtags": [
      "Agente",
      "Auto",
      "Traslados"
    ...
    "comentarios": 7,
    "compartidos": 13,
    "plantilla": "Tengo auto y puedo apoyar con traslados, paquetes medianos o compras dentro de la zona. Busco acuerdos claros y trato directo."
  },
  {
    "id": "v4921-agente-acompanamiento",
    "tipo": "agentes",
    "nombre": "Elena Cruz",
    "rol": "Agente",
    "ubicacion": "CDMX",
    "titulo": "Acompaño a adultos mayores",
    "descripcion": "Ofrezco apoyo para citas, compras sencillas y acompañamiento por zona.",
    "hashtags": [
      "Agente",
      "Apoyo",
      "Confianza"
    ...
    "comentarios": 5,
    "compartidos": 10,
    "plantilla": "Ofrezco apoyo y acompañamiento para adultos mayores en compras, citas o trámites sencillos. Trabajo por zona y con horarios acordados."
  },
  {
    "id": "v4921-negocio-panaderia",
    "tipo": "negocios",
    "nombre": "Panadería La Esquina",
    "rol": "Negocio local",
    "ubicacion": "Toluca, México",
    "titulo": "Panadería recibe pedidos por chat",
    "descripcion": "Pan dulce, bolillo y pedidos para recoger. Atención directa y horarios claros.",
    "hashtags": [
      "Negocio",
      "Panadería",
      "Pickup"
    ...
    "comentarios": 4,
    "compartidos": 9,
    "plantilla": "Tengo una panadería local y quiero recibir pedidos por chat, mostrar productos, horarios y coordinar pickup o entrega cercana."
  },
  {
    "id": "v4921-negocio-estetica",
    "tipo": "negocios",
    "nombre": "Estética Bella Zona",
    "rol": "Negocio local",
    "ubicacion": "Metepec, México",
    "titulo": "Estética agenda citas por WhatsApp",
    "descripcion": "Servicios de corte, uñas y peinado con disponibilidad por horario.",
    "hashtags": [
      "Negocio",
      "Citas",
      "Belleza"
    ...
    "comentarios": 6,
    "compartidos": 10,
    "plantilla": "Tengo una estética y quiero organizar citas, servicios, horarios y contacto por chat para que mis clientes puedan agendar fácilmente."
  },
  {
    "id": "v4921-negocio-ferreteria",
    "tipo": "negocios",
    "nombre": "Ferretería San José",
    "rol": "Negocio local",
    "ubicacion": "Chapultepec, México",
    "titulo": "Ferretería cotiza y entrega cerca",
    "descripcion": "Herramientas, material básico y cotizaciones rápidas para vecinos.",
    "hashtags": [
      "Negocio",
      "Cotización",
      "Herramientas"
    ...
    "comentarios": 5,
    "compartidos": 8,
    "plantilla": "Tengo una ferretería y quiero recibir consultas de productos, cotizaciones y pedidos para entrega cercana o pickup."
  },
  {
    "id": "v4921-negocio-carpinteria",
    "tipo": "negocios",
    "nombre": "Carpintería Los Pinos",
    "rol": "Negocio local",
    "ubicacion": "San Mateo, México",
    "titulo": "Carpintería acepta cotizaciones",
    "descripcion": "Fabricación y reparación sencilla de muebles con revisión por zona.",
    "hashtags": [
      "Negocio",
      "Carpintería",
      "Cotización"
    ...
    "comentarios": 6,
    "compartidos": 12,
    "plantilla": "Tengo una carpintería y quiero recibir solicitudes de cotización, fotos de referencia y datos de contacto para acordar trabajos."
  },
  {
    "id": "v4921-negocio-restaurante",
    "tipo": "negocios",
    "nombre": "Cocina Doña Lety",
    "rol": "Negocio local",
    "ubicacion": "Toluca, México",
    "titulo": "Comida casera para recoger",
    "descripcion": "Menú del día, pedidos por chat y opción de recoger en el local.",
    "hashtags": [
      "Negocio",
      "Comida",
      "Pickup"
    ...
    "comentarios": 9,
    "compartidos": 16,
    "plantilla": "Tengo un negocio de comida y quiero mostrar menú, recibir pedidos por chat y coordinar pickup o entrega local."
  },
  {
    "id": "v4921-servicio-reparacion",
    "tipo": "servicios",
    "nombre": "Luis Hernández",
    "rol": "Servicio",
    "ubicacion": "Metepec, México",
    "titulo": "Reparación de electrodomésticos",
    "descripcion": "Reviso lavadoras, licuadoras, hornos y fallas sencillas a domicilio.",
    "hashtags": [
      "Servicio",
      "Reparación",
      "Hogar"
    ...
    "comentarios": 3,
    "compartidos": 7,
    "plantilla": "Ofrezco revisión y reparación básica de electrodomésticos a domicilio. Indico zona, horarios y condiciones para trato directo."
  },
  {
    "id": "v4921-servicio-limpieza",
    "tipo": "servicios",
    "nombre": "Claudia Reyes",
    "rol": "Servicio",
    "ubicacion": "Toluca, México",
    "titulo": "Limpieza por día o por horas",
    "descripcion": "Apoyo en limpieza de casa, oficina o local con horario acordado.",
    "hashtags": [
      "Servicio",
      "Limpieza",
      "Disponible"
    ...
    "comentarios": 6,
    "compartidos": 11,
    "plantilla": "Ofrezco servicio de limpieza por día o por horas. Puedo trabajar por zona, con horario acordado y trato directo."
  },
  {
    "id": "v4921-servicio-computacion",
    "tipo": "servicios",
    "nombre": "Mónica Salas",
    "rol": "Servicio",
    "ubicacion": "Chapultepec, México",
    "titulo": "Clases de computación para adultos",
    "descripcion": "Enseño uso básico de celular, computadora, redes sociales y aplicaciones.",
    "hashtags": [
      "Servicio",
      "Computación",
      "Aprendizaje"
    ...
    "comentarios": 7,
    "compartidos": 15,
    "plantilla": "Ofrezco clases básicas de celular y computadora para adultos. Enseño paso a paso redes sociales, aplicaciones y herramientas útiles."
  },
  {
    "id": "v4921-servicio-electricidad",
    "tipo": "servicios",
    "nombre": "Raúl Ortega",
    "rol": "Servicio",
    "ubicacion": "Mexicaltzingo, México",
    "titulo": "Instalación eléctrica sencilla",
    "descripcion": "Cambio contactos, apagadores, lámparas y reparaciones básicas por zona.",
    "hashtags": [
      "Servicio",
      "Electricidad",
      "Hogar"
    ...
    "comentarios": 5,
    "compartidos": 10,
    "plantilla": "Ofrezco instalación eléctrica sencilla: contactos, apagadores, lámparas y revisiones básicas. Trabajo por zona y con presupuesto acordado."
  },
  {
    "id": "v4921-servicio-mascotas",
    "tipo": "servicios",
    "nombre": "Paseos Caninos Leo",
    "rol": "Servicio",
    "ubicacion": "CDMX",
    "titulo": "Paseos y cuidado de mascotas",
    "descripcion": "Paseos por horario, cuidado temporal y apoyo a vecinos con mascotas.",
    "hashtags": [
      "Servicio",
      "Mascotas",
      "Vecinos"
    ...
    "comentarios": 9,
    "compartidos": 14,
    "plantilla": "Ofrezco paseos y cuidado de mascotas por horario. Puedo coordinar zona, frecuencia y condiciones directamente con el dueño."
  },
  {
    "id": "v4921-crecimiento-creditos",
    "tipo": "crecimiento",
    "nombre": "Campaña local",
    "rol": "Crecimiento",
    "ubicacion": "Estado de México",
    "titulo": "Busco clientes para servicio financiero",
    "descripcion": "Campaña por resultado con reglas claras, contacto válido y comisión acordada.",
    "hashtags": [
      "Crecimiento",
      "Clientes",
      "Comisión"
    ...
    "comentarios": 11,
    "compartidos": 18,
    "plantilla": "Busco personas que me ayuden a conseguir clientes interesados para un servicio. Quiero definir resultado válido, comisión, zona y condiciones claras."
  },
  {
    "id": "v4921-crecimiento-restaurante",
    "tipo": "crecimiento",
    "nombre": "Restaurante en crecimiento",
    "rol": "Crecimiento",
    "ubicacion": "Toluca, México",
    "titulo": "Necesito más pedidos por semana",
    "descripcion": "Busco promotores digitales que ayuden a mover menú y pedidos por comisión.",
    "hashtags": [
      "Crecimiento",
      "Pedidos",
      "Promoción"
    ...
    "comentarios": 8,
    "compartidos": 13,
    "plantilla": "Tengo un negocio y quiero conseguir más pedidos por semana. Busco agentes o promotores que ayuden con difusión por resultado."
  },
  {
    "id": "v4921-crecimiento-redes",
    "tipo": "crecimiento",
    "nombre": "Marca local",
    "rol": "Crecimiento",
    "ubicacion": "Metepec, México",
    "titulo": "Busco quien publique mi negocio",
    "descripcion": "Necesito apoyo con flyers, estados de WhatsApp y publicaciones locales.",
    "hashtags": [
      "Crecimiento",
      "Redes",
      "Marketing"
    ...
    "comentarios": 12,
    "compartidos": 19,
    "plantilla": "Necesito apoyo para promocionar mi negocio en redes, estados de WhatsApp y grupos locales. Quiero pagar por resultado o acuerdo claro."
  },
  {
    "id": "v4921-crecimiento-ventas",
    "tipo": "crecimiento",
    "nombre": "Tienda local",
    "rol": "Crecimiento",
    "ubicacion": "Chapultepec, México",
    "titulo": "Pago comisión por cada venta",
    "descripcion": "Busco personas que recomienden productos y ganen por venta concretada.",
    "hashtags": [
      "Crecimiento",
      "Ventas",
      "Referidos"
    ...
    "comentarios": 10,
    "compartidos": 18,
    "plantilla": "Busco personas que recomienden mis productos y ganen comisión por cada venta concretada. Quiero registrar condiciones y seguimiento."
  },
  {
    "id": "v4921-crecimiento-citas",
    "tipo": "crecimiento",
    "nombre": "Consultorio local",
    "rol": "Crecimiento",
    "ubicacion": "CDMX",
    "titulo": "Necesito citas reales para mi servicio",
    "descripcion": "Pago por cita válida y cliente interesado, con reglas claras de validación.",
    "hashtags": [
      "Crecimiento",
      "Citas",
      "Resultado"
    ...
    "comentarios": 8,
    "compartidos": 14,
    "plantilla": "Necesito generar citas reales para mi servicio. Quiero acordar pago por cita válida, zona, requisitos y forma de validación."
  },
  {
    "id": "v4921-embajador-comision",
    "tipo": "embajadores",
    "nombre": "Embajadores Conecta",
    "rol": "Embajadores",
    "ubicacion": "Todo México",
    "titulo": "Gana $50 por membresía referida",
    "descripcion": "Membresía anual $98 MXN. Recomienda Conecta y gana por activación pagada.",
    "hashtags": [
      "Embajadores",
      "Membresía",
      "Ingresos"
    ...
    "comentarios": 14,
    "compartidos": 21,
    "plantilla": "Quiero ser embajador Conecta para recomendar la app, registrar referidos y ganar comisión por membresías anuales pagadas."
  },
  {
    "id": "v4921-embajador-negocios",
    "tipo": "embajadores",
    "nombre": "Activación de negocios",
    "rol": "Embajadores",
    "ubicacion": "Municipios",
    "titulo": "Invita negocios de tu colonia",
    "descripcion": "Ayuda a negocios locales a entrar a Conecta y gana por membresía activada.",
    "hashtags": [
      "Embajadores",
      "Negocios",
      "Comisión"
    ...
    "comentarios": 12,
    "compartidos": 20,
    "plantilla": "Quiero invitar negocios locales a Conecta Servicios y explicarles cómo publicar, aparecer y contactar clientes."
  },
  {
    "id": "v4921-embajador-vecinos",
    "tipo": "embajadores",
    "nombre": "Comunidad Conecta",
    "rol": "Embajadores",
    "ubicacion": "Estado de México",
    "titulo": "Ayuda a vecinos a publicar",
    "descripcion": "Orienta a personas que necesitan ayuda, servicios o ingresos locales.",
    "hashtags": [
      "Embajadores",
      "Vecinos",
      "Comunidad"
    ...
    "comentarios": 10,
    "compartidos": 16,
    "plantilla": "Quiero ayudar a vecinos a publicar necesidades, servicios o negocios dentro de Conecta Servicios y registrar mis referidos."
  },
  {
    "id": "v4921-embajador-municipio",
    "tipo": "embajadores",
    "nombre": "Embajador municipal",
    "rol": "Embajadores",
    "ubicacion": "Chapultepec, México",
    "titulo": "Activa tu municipio con Conecta",
    "descripcion": "Comparte la app, registra miembros y ayuda a crear movimiento local.",
    "hashtags": [
      "Embajadores",
      "Municipio",
      "Activación"
    ...
    "comentarios": 15,
    "compartidos": 24,
    "plantilla": "Quiero activar mi municipio como embajador, invitar comercios, agentes, servicios y personas que quieran publicar."
  },
  {
    "id": "v4921-embajador-manual",
    "tipo": "embajadores",
    "nombre": "Manual Conecta",
    "rol": "Embajadores",
    "ubicacion": "Desde tu celular",
    "titulo": "Aprende a explicar la app",
    "descripcion": "Guía rápida para entender publicaciones, membresías, referidos y comisiones.",
    "hashtags": [
      "Embajadores",
      "Manual",
      "Aprender"
    ...
    "comentarios": 7,
    "compartidos": 12,
    "plantilla": "Quiero aprender cómo explicar Conecta Servicios, cómo registrar referidos y cómo orientar a nuevos miembros."
  },
  {
    "id": "v4921-aprendizaje-computacion",
    "tipo": "aprendizaje",
    "nombre": "Aprendizaje Conecta",
    "rol": "Aprendizaje",
    "ubicacion": "Desde tu celular",
    "titulo": "Computación básica para trabajar",
    "descripcion": "Aprende celular, computadora, documentos y redes para mejorar tus oportunidades.",
    "hashtags": [
      "Aprendizaje",
      "Computación",
      "Ingresos"
    ...
    "comentarios": 6,
    "compartidos": 13,
    "plantilla": "Quiero aprender computación básica para usar celular, documentos, aplicaciones y redes sociales de forma útil para trabajar o emprender."
  },
  {
    "id": "v4921-aprendizaje-whatsapp",
    "tipo": "aprendizaje",
    "nombre": "Ventas por WhatsApp",
    "rol": "Aprendizaje",
    "ubicacion": "Online",
    "titulo": "Aprende a vender por WhatsApp",
    "descripcion": "Organiza mensajes, catálogo, seguimiento y trato con clientes.",
    "hashtags": [
      "Aprendizaje",
      "Ventas",
      "WhatsApp"
    ...
    "comentarios": 9,
    "compartidos": 17,
    "plantilla": "Quiero aprender a vender por WhatsApp: responder clientes, organizar pedidos, dar seguimiento y cerrar ventas con mejor comunicación."
  },
  {
    "id": "v4921-aprendizaje-oficios",
    "tipo": "aprendizaje",
    "nombre": "Oficios útiles",
    "rol": "Aprendizaje",
    "ubicacion": "Online",
    "titulo": "Aprende un oficio y ofrece servicio",
    "descripcion": "Conecta capacitación con servicios locales que puedas publicar después.",
    "hashtags": [
      "Aprendizaje",
      "Oficios",
      "Servicio"
    ...
    "comentarios": 7,
    "compartidos": 15,
    "plantilla": "Quiero aprender un oficio útil para después ofrecerlo como servicio local dentro de Conecta Servicios."
  },
  {
    "id": "v4921-aprendizaje-fotos",
    "tipo": "aprendizaje",
    "nombre": "Mejora tus publicaciones",
    "rol": "Aprendizaje",
    "ubicacion": "Desde tu celular",
    "titulo": "Toma mejores fotos para vender",
    "descripcion": "Aprende a mostrar productos, servicios o negocios con imágenes más claras.",
    "hashtags": [
      "Aprendizaje",
      "Fotos",
      "Negocio"
    ...
    "comentarios": 6,
    "compartidos": 14,
    "plantilla": "Quiero aprender a tomar mejores fotos de productos, servicios o negocios para que mis publicaciones generen más confianza."
  },
  {
    "id": "v4921-aprendizaje-atencion",
    "tipo": "aprendizaje",
    "nombre": "Atención al cliente",
    "rol": "Aprendizaje",
    "ubicacion": "Online",
    "titulo": "Aprende a tratar mejor al cliente",
    "descripcion": "Mensajes claros, seguimiento, confianza y cierre de acuerdos.",
    "hashtags": [
      "Aprendizaje",
      "Cliente",
      "Confianza"
    ...
    "comentarios": 8,
    "compartidos": 16,
    "plantilla": "Quiero aprender atención al cliente para responder mejor, dar seguimiento y generar confianza en servicios, negocios o ventas."
  }
];
...
let lastPublicationMediaUploadV4931 = null;
let lastEditMediaUploadV4931 = null;
...
function normalizeStoragePathV4931(value = "", bucket = "") {
  ...
  text = text.replace(/^\/storage\/v1\/object\/public\//, "");
  if (bucket && text.startsWith(`${bucket}/`)) text = text.slice(bucket.length + 1);
  return text.replace(/^\/+/, "");
}
function storagePublicUrlV4930(bucket, path) {
  const cleanPath = normalizeStoragePathV4931(path, bucket);
  if (!cleanPath) return "";
  if (/^https?:\/\//i.test(cleanPath)) return cleanPath;
  try {
    const storageClient = window.supabase?.storage;
    if (storageClient && typeof storageClient.from === "function") {
      const publicUrl = storageClient.from(bucket).getPublicUrl(cleanPath);
      const url = publicUrl?.data?.publicUrl;
      if (url) return url;
    }
  } catch (error) {
    console.debug("getPublicUrl no disponible; usando URL pública manual.", error.message);
  }
  return `${SUPABASE_URL}/storage/v1/object/public/${bucket}/${cleanPath.split("/").map(encodeURIComponent).join("/")}`;
}
function displayMediaUrlV4931(value = "", bucket = "") {
  const text = String(value || "").trim();
  if (!text) return "";
  if (/^(https?:|data:|blob:)/i.test(text)) return text;
  if (/^(assets\/|\.\/assets\/|\/assets\/)/i.test(text)) return text;
  return storagePublicUrlV4930(bucket, text);
}
...
function showMediaPreviewV4930(containerId, fileOrUrl) {
  ...
  box.innerHTML = `<img src="${safeUrl}" alt="Vista previa" onerror="this.onerror=null;this.src='assets/photo4922-servicios-1.webp';" /><small>Vista previa de la foto principal</small>`;
}
...
async function uploadSelectedPublicationMediaIfNeededV4930() {
  if (!selectedPublicationMediaFileV4930) {
    lastPublicationMediaUploadV4931 = null;
    return String(document.getElementById("pubMediaUrl")?.value || "").trim();
  }
  ...
  lastPublicationMediaUploadV4931 = uploaded;
...
function formPayload() {
  ...
  if (isLikelyMediaUrl(mediaUrl)) {
    ...
    payload.media_path = lastPublicationMediaUploadV4931?.path || "";
    payload.media_created_at = new Date().toISOString();
  }
  return payload;
}
...
async function saveEdit(event) {
  ...
  try {
    if (selectedEditMediaFileV4930) {
      ...
      lastEditMediaUploadV4931 = uploaded;
  ...
  if (isLikelyMediaUrl(mediaUrl)) {
    payload.image_url = mediaUrl;
    payload.media_type = lastEditMediaUploadV4931?.type || "image";
    payload.media_path = lastEditMediaUploadV4931?.path || "";
    payload.media_created_at = new Date().toISOString();
  }
  try {
    await patchPublicationWithMediaFallbackV4931(id, payload);
...
async function saveRemotePilotFeedOverrideV4930(id, imageUrl, objectPosition) {
  const base = { template_id: id, object_position: objectPosition || "center center", updated_at: new Date().toISOString() };
  const preferred = [{ ...base, image_url_override: imageUrl }];
  const legacy = [{ ...base, image_url: imageUrl }];
  try {
    ...
    try {
      ...
      console.debug("Overrides remotos no disponibles; se conserva override local.", fallbackError.message || error.message);
    }
  }
}
...
function renderPilotFeedAdminEditor() {
  ...
  panel.innerHTML = `<div class="notice-card pilot-admin-editor"><strong>Editar multimedia del Home Feed piloto</strong><p>Sube una imagen al bucket <b>template-media</b> o pega una URL pública. La app guarda la URL pública completa como <b>image_url_override</b> de la plantilla.</p>${rows}<p class="field-help">Para que estos cambios se vean en todos los dispositivos, crea la tabla opcional <b>${TEMPLATE_OVERRIDES_TABLE_V4930}</b> o conserva los cambios en código en la siguiente versión.</p></div>`;
}
...
const MEMBER_ACCESS_MY_PUBLICATIONS_VERSION_V49362 = "4.9.39-asistente-publicaciones";
...
try {
  ...
  console.warn("No se pudo reforzar renderSurveyFlow para Asistente de publicación", error);
}
...
try {
  ...
  console.warn("No se pudo reforzar openPilotFlow para Asistente de publicación", error);
}
...
try {
  ...
  console.warn("No se pudo reforzar startSurveyFlowFromPilot para Asistente de publicación", error);
}
...
try {
  ...
  console.warn("No se pudo reemplazar almacenamiento de Asistente de publicación", error);
}
...
try {
  ...
  console.warn("No se pudo reforzar guardado de Asistente de publicación", error);
}
  ...
  return `
    <article class="business-publication-card-v49367">
      ...
      <div class="business-publication-card-v49367__actions">
        <button type="button" onclick="openBusinessClientFilterV49367('${safeTextV49367(publication.id)}')">Mensaje al publicante</button>
  ...
  if (!rows.length) {
    panel.innerHTML = mode === "owner" ? `
      <div class="business-publications-empty-v49367">
        ...
        <p>Cuando registres un Asistente de publicación, aquí aparecerá como publicación para que el cliente pueda contactarte usando un filtro.</p>
      </div>` : "";
    return;
  }
  panel.innerHTML = `
...
try {
  ...
  console.warn("No se pudo reemplazar guardado de Asistente de publicación v4.9.36.7", error);
}
...
// ------------------------------------------------------------------
// v4.9.36.8 — Asistente de publicación con membresía obligatoria + Admin libre
// Objetivo:
// 1) Si un usuario crea un Asistente de publicación, al finalizar debe ver la invitación
...
// ------------------------------------------------------------------
const BUSINESS_CHAT_MEMBERSHIP_VERSION_V49368 = "4.9.39-asistente-publicaciones";
...
function renderBusinessChatMembershipRequiredV49368(draft = {}) {
  ...
  root.innerHTML = `
    <div class="business-chat-membership-gate-v49368">
      ...
      <span>Asistente de publicación listo</span>
  ...
  try {
    if (typeof saveUserAccessProfileV4936 === "function") {
      saveUserAccessProfileV4936({
        ...
        notes: `Asistente de publicación pendiente: ${draft.goal || draft.type || "Servicio"}`
      });
    }
    if (typeof saveUserAccessHistoryV4936 === "function") {
      saveUserAccessHistoryV4936({ action: "Asistente de publicación pendiente", note: `${draft.name || "Negocio"} · requiere membresía` });
    }
  } catch {}
...
function renderPendingBusinessChatNoticeV49368() {
  ...
  panel.innerHTML = `
    <span>💬 Asistente de publicación pendiente</span>
...
try {
  ...
  console.warn("No se pudo agregar gate al Asistente de publicación v4.9.36.8", error);
}
...
// Sustituye el viejo "Asistente de publicación / Mensaje al publicante" por flujos internos
...
(function businessMessagingV4937(){
  ...
  function renderConfigPage(context = {}) {
    ...
    root.innerHTML = `
      ...
      <span class="bm-kicker-v4937">v4.9.39 · Asistente de publicación</span>
      <h2>Configurar mensajería de negocio</h2>
      <p class="bm-subtitle-v4937">Este flujo sustituye el antiguo “Mensaje al publicante”. Ahora el publicante decide si quiere recibir pedidos, citas o mensajes internos por publicación.</p>
  ...
  function overrideLegacy() {
    // Redirige el viejo Asistente de publicación al nuevo configurador.
    try {
...
// ------------------------------------------------------------------
(function chatbotConectaSocialV4944(){
  const VERSION = "v4.9.44-chatbot-conecta-publicacion-social";
  const FREE_TRIAL_KEY = "conecta_free_trial_publication_v4944";
  const LOCAL_PUBLICATIONS_KEY = "conecta_publicaciones_basicas_locales_v4943";
  ...
  const PROFILE_TYPES = {
    ...
    agente: { label: "Agente", icon: "🚀", hint: "Ofrezco hacer mandados, entregas, viajes, trámites o apoyo local." },
    negocio: { label: "Negocio", icon: "🏪", hint: "Tengo un negocio, producto o servicio y quiero recibir pedidos, citas o cotizaciones." }
  };
  const CHATBOT_MODES = {
    solicitante: ["¿Qué necesitas?", "¿En qué zona se realiza?", "¿Para cuándo lo necesitas?", "¿Tienes presupuesto o pago sugerido?"],
    agente: ["¿Qué tipo de servicio o mandado te piden?", "¿En qué zona puedes atender?", "¿En qué horario estás disponible?", "¿Cómo quieres acordar el costo?"],
    negocio: ["¿Qué producto, servicio o cita necesita el cliente?", "¿Cantidad o alcance aproximado?", "¿Entrega, pickup, cita o contacto?", "¿Horario preferido?"],
  ...
  function read(key, fallback){ try { const raw=localStorage.getItem(key); if(!raw) return fallback; const parsed=JSON.parse(raw); return parsed ?? fallback; } catch { return fallback; } }
  function write(key, value){ try { localStorage.setItem(key, JSON.stringify(value)); } catch {} }
  function uid(prefix="CS"){ return `${prefix}-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(16).slice(2,7).toUpperCase()}`; }
  function formValue(id){ return qs(`#${id}`)?.value?.trim?.() || ""; }
  function descriptionValue(){
    const ids=["pubDescription","pubDescriptionRide","pubDetails","publicationDescription"];
    for (const id of ids){ const v=formValue(id); if(v) return v; }
    const ta=qs("#publicationForm textarea"); return ta?.value?.trim?.() || "";
  }
  function isAdmin(){
    try { if (typeof isAdminSupervisorActiveV49368 === "function" && isAdminSupervisorActiveV49368()) return true; } catch {}
    try { if (typeof adminAccessAvailableV4936 === "function" && adminAccessAvailableV4936()) return true; } catch {}
  ...
  function freeTrial(){ return read(FREE_TRIAL_KEY, null); }
  function freeTrialActive(){
    const trial=freeTrial();
    if(!trial || !trial.expires_at) return false;
    return new Date(trial.expires_at).getTime() > Date.now();
  }
  ...
  function recordFreeTrial(saved){
    ...
    const existing=freeTrial();
    if (existing?.used && freeTrialActive()) return;
    const created=new Date();
    write(FREE_TRIAL_KEY, {
      used:true,
      id:saved.id || saved.local_id || uid("FREE"),
      title:saved.titulo || saved.title || "Publicación gratis",
      created_at:created.toISOString(),
      expires_at:addDays(created, TRIAL_DAYS).toISOString(),
      version:VERSION
    });
  }
  function normalizeProfileFromCategory(category="", intent=""){
    const c=String(category||"").toLowerCase();
    const i=String(intent||"").toLowerCase();
    if (/negocio|alimento|venta|tienda|comercio|consultor|profesional/.test(c)) return "negocio";
    if (/agente|mandado|entrega|viaje|mensajer|envío|movilidad/.test(c)) return "agente";
    if (/busco|necesito|solicit/.test(i)) return "solicitante";
    return "solicitante";
  }
  function profileChoice(){ return qs("#publicationProfileTypeV4944")?.value || normalizeProfileFromCategory(formValue("pubCategory"), formValue("pubIntent")); }
  function contactChoice(){ return qs("input[name='contactChannelV4944']:checked")?.value || "chatbot"; }
  function chatbotTypeChoice(){ return qs("#publicationChatbotTypeV4944")?.value || profileChoice() || "general"; }
  function channelFromItem(item={}){
    const reference=String(item.reference || item.referencia || "").toLowerCase();
    if (/contact_channel\s*=\s*whatsapp|canal\s*:\s*whatsapp|whatsapp principal/.test(reference)) return "whatsapp";
    if (item.contact_channel === "whatsapp") return "whatsapp";
    return "chatbot";
  }
  function profileFromItem(item={}){
    const reference=String(item.reference || item.referencia || "").toLowerCase();
    const m=reference.match(/perfil_publicacion\s*=\s*(solicitante|agente|negocio)/);
    if (m) return m[1];
    return normalizeProfileFromCategory(item.category || item.categoria_principal, item.intent || item.intencion || item.title);
  }
  ...
  function controlsHtml(){
    const active=hasMembership() || isAdmin();
    const canFree=!freeTrialUsed() || !freeTrialActive();
    const status=trialMessage();
    return `<section id="chatbotConectaPublishPanelV4944" class="chatbot-publish-panel-v4944">
      ...
      <label class="chatbot-field-v4944">
        <span>¿Quién publica?</span>
        <select id="publicationProfileTypeV4944">
          ${Object.entries(PROFILE_TYPES).map(([key, val]) => `<option value="${key}">${val.icon} ${val.label}</option>`).join("")}
        </select>
        <small>Usamos solo tres perfiles para evitar confusión: Solicitante, Agente o Negocio.</small>
      </label>
      ...
      <label class="chatbot-field-v4944" id="chatbotModeFieldV4944">
        ...
        <select id="publicationChatbotTypeV4944">
          ...
          <option value="negocio">Pedidos, citas, cotizaciones o mensajes de negocio</option>
  ...
  function ensurePublishControls(){
    const form=qs("#publicationForm");
    if(!form) return;
    qsa("#publicationAssistantGateV4943,#publicationAssistantGateV4942,#publicationAssistantGateV4940,#publicationAssistantPanelV4939").forEach(el => { el.style.display="none"; });
    let panel=qs("#chatbotConectaPublishPanelV4944", form);
    if(!panel){
      const wrap=document.createElement("div");
      wrap.innerHTML=controlsHtml();
      panel=wrap.firstElementChild;
      const anchor=qs("#publicationMembershipGate", form) || qs(".wizard-review", form) || form.querySelector("button[type='submit']") || form.lastElementChild;
      if(anchor?.parentElement) anchor.parentElement.insertBefore(panel, anchor);
      else form.appendChild(panel);
    }
    const title=qs("#publicationFormTitle") || qs("#registro h2") || qs("#registro h1");
    if(title && /crear oportunidad|publicar/i.test(title.textContent||"")) title.textContent="Crear publicación";
    const phone=qs("#pubPhone");
    if(phone){ phone.required=false; phone.placeholder="WhatsApp opcional si eliges WhatsApp"; }
    const category=qs("#pubCategory");
    if(category && category.dataset.v4944Normalized!=="1"){
      category.dataset.v4944Normalized="1";
      qsa("option", category).forEach(opt => {
        if (/^Servicios$/i.test(opt.textContent||"")) opt.textContent="Negocio o agente independiente";
      });
    }
    const syncVisibility=()=>{
      const channel=contactChoice();
      const mode=qs("#chatbotModeFieldV4944");
      if(mode) mode.style.display = channel === "whatsapp" ? "none" : "grid";
      if(phone){
        phone.required = channel === "whatsapp";
        phone.closest("label")?.classList?.toggle("required-channel-v4944", channel === "whatsapp");
      }
      const profile=profileChoice();
      const chatbotSelect=qs("#publicationChatbotTypeV4944");
      if(chatbotSelect && chatbotSelect.value === "general") chatbotSelect.value=profile;
    };
    qsa("input[name='contactChannelV4944'],#publicationProfileTypeV4944", panel).forEach(el => { if(el.dataset.v4944Bound!=="1"){ el.dataset.v4944Bound="1"; el.addEventListener("change", syncVisibility); }});
    syncVisibility();
  }

  function buildPayload(){
    let raw={};
    try { if (typeof formPayload === "function") raw=formPayload() || {}; } catch (error) { console.warn("formPayload falló en v4.9.44; se leerá el formulario directo", error); }
    const profile=profileChoice();
    const channel=contactChoice();
    const chatbotMode=channel === "chatbot" ? chatbotTypeChoice() : "none";
    const phone=cleanPhoneLocal(raw.telefono || formValue("pubPhone"));
    const title=raw.titulo || formValue("pubTitle");
    const description=raw.descripcion || descriptionValue();
    const category=raw.categoria_principal || formValue("pubCategory") || (profile === "negocio" ? "Negocios locales" : profile === "agente" ? "Agentes de crecimiento" : "Solicitantes");
    const referenceParts=[
      raw.referencia || "",
      VERSION,
      `perfil_publicacion=${profile}`,
      `contact_channel=${channel}`,
      `chatbot_conecta=${channel === "chatbot" ? "activo" : "sustituido_por_whatsapp"}`,
      `chatbot_tipo=${chatbotMode}`,
      hasMembership() || isAdmin() ? "modelo=membresia_ilimitada" : "modelo=gratis_30_dias"
    ].filter(Boolean);
    return {
      nombre_publico: raw.nombre_publico || formValue("pubName") || "Usuario Conecta",
      titulo: title,
      descripcion: description,
      categoria_principal: category,
      subcategoria: raw.subcategoria || "",
      intencion: raw.intencion || formValue("pubIntent") || profile,
      estado_nombre: raw.estado_nombre || formValue("pubState") || "México",
      municipio: raw.municipio || formValue("pubMunicipality"),
      localidad: raw.localidad || formValue("pubLocality"),
      telefono: channel === "whatsapp" ? phone : (phone || ""),
      ruta: raw.ruta || formValue("pubRoute"),
      salida: raw.salida || formValue("pubDeparture"),
      hora: raw.hora || formValue("pubTime"),
      medio: raw.medio || formValue("pubTransport"),
      ...
      radio_servicio_km: raw.radio_servicio_km || 10,
      estado: "activo",
      referencia: referenceParts.join(" · ").slice(0, 900),
      image_url: raw.image_url || "",
      media_type: raw.media_type || "",
      media_path: raw.media_path || "",
      media_created_at: raw.media_created_at || "",
      profile_type: profile,
      contact_channel: channel,
      chatbot_conecta: channel === "chatbot",
      chatbot_type: chatbotMode,
      source_version: VERSION
    };
  }
  function validatePayload(payload){
    ...
    if(!isAdmin() && !hasMembership()){
      const trial=freeTrial();
      if(trial?.used){
        if(freeTrialActive()) toast("Ya tienes una publicación gratis activa por 30 días. Activa la membresía anual de $98 para publicar sin límite.");
        else toast("Tu publicación gratis de 30 días ya venció. Activa la membresía anual de $98 para seguir publicando.");
        return false;
      }
    }
    return true;
  }
  const REMOTE_FIELDS=["nombre_publico","titulo","descripcion","categoria_principal","subcategoria","intencion","estado_nombre","municipio","localidad","telefono","ruta","salida","hora","medio","presupuesto","latitud","longitud","ubicacion_aproximada","radio_servicio_km","estado","referencia","image_url","media_type","media_path","media_created_at"];
  function remotePayload(payload){ const out={}; REMOTE_FIELDS.forEach(k => { if(payload[k] !== undefined && payload[k] !== null && payload[k] !== "") out[k]=payload[k]; }); return out; }
  function localRows(){ return read(LOCAL_PUBLICATIONS_KEY, []); }
  function saveLocal(payload, status="local"){
    const id=payload.id || payload.local_id || uid("PUB");
    const row={...payload, id, local_id:id, sync_status:status, created_at:payload.created_at || now(), updated_at:now()};
    write(LOCAL_PUBLICATIONS_KEY, [row, ...localRows().filter(x => x.id !== id && x.local_id !== id)].slice(0,100));
    try {
      const mapped=typeof mapPublication === "function" ? mapPublication(row) : null;
      if(mapped){ mapped.contact_channel=payload.contact_channel; mapped.profile_type=payload.profile_type; mapped.chatbot_type=payload.chatbot_type; }
      if(mapped && typeof publicationsCache !== "undefined" && Array.isArray(publicationsCache)) publicationsCache=[mapped, ...publicationsCache.filter(p => p.id !== mapped.id)];
    } catch {}
    return row;
  }
  async function submitV4944(event){
    const form=qs("#publicationForm");
    ...
    try { if (typeof renderMyPublications === "function") renderMyPublications(); } catch {}
    try { if (typeof showSection === "function") showSection("misPublicaciones"); } catch {}
    setTimeout(() => { renderChatbotResponsesPanel(); renderStarSections(); decorateSocialButtons(); ensurePublishControls(); }, 120);
    return false;
  }

  function contactButton(item){
    const channel=channelFromItem(item);
    if(channel === "whatsapp") return `<button class="btn-small btn-green contact-main-v4944" type="button" onclick="contactPublication('${esc(item.id)}')">🟢 WhatsApp</button>`;
    return `<button class="btn-small btn-purple contact-main-v4944" type="button" onclick="openConectaChatbotV4944('${esc(item.id)}')">💬 ${chatbotLabelForItem(item)}</button>`;
  }
  function removeLegacyContactHtml(html){
    return String(html || "")
      .replace(/<button[^>]+onclick="contactPublication\('[^']+'\)"[^>]*>[\s\S]*?<\/button>/g, "")
      .replace(/<button[^>]+onclick="configurePublicationAssistantForItemV4939[\s\S]*?<\/button>/g, "")
      .replace(/<button[^>]*>\s*(?:Responder filtro|Chat negocio|Abrir chat de negocios|Contactar)\s*<\/button>/gi, "");
  }
  function installCardWrappers(){
    try {
      if (typeof compactPublicationCard === "function" && !window.__compactPublicationCardBaseV4944) {
        window.__compactPublicationCardBaseV4944=compactPublicationCard;
        compactPublicationCard=function(item){
          let html=window.__compactPublicationCardBaseV4944(item);
          html=removeLegacyContactHtml(html);
          html=html.replace(/(<button class="btn-small btn-green" onclick="openRealPublicationFeedDetail\('[^']+'\)">Ver publicación<\/button>)/, `$1 ${contactButton(item)}`);
          html=html.replace(/(<div class="detail-actions">)/, `$1 ${contactButton(item)}`);
          html=html.replace(/Publicar algo parecido/g, "Publicar algo parecido");
          return html;
        };
      }
    } catch(error){ console.warn("No se pudo envolver compactPublicationCard v4.9.44", error); }
    try {
      if (typeof myPublicationCard === "function" && !window.__myPublicationCardBaseV4944) {
    ...
    } catch(error){ console.warn("No se pudo envolver myPublicationCard v4.9.44", error); }
    try {
      if (typeof homeFeedPostCardV4918 === "function" && !window.__homeFeedPostCardBaseV4944) {
        window.__homeFeedPostCardBaseV4944=homeFeedPostCardV4918;
        homeFeedPostCardV4918=function(post){
          let html=window.__homeFeedPostCardBaseV4944(post);
          html=html.replace(/Guardar plantilla/g,"Publicar algo parecido").replace(/>▢ <small>Guardar<\/small>/g,">＋ <small>Publicar parecido</small>");
          html=html.replace(/savePilotTemplate\('/g,"makePilotReal('");
          return html;
        };
      }
    } catch(error){ console.warn("No se pudo envolver homeFeedPostCard v4.9.44", error); }
  }

  function findPublication(id){
    try { return (publicationsCache || []).find(p => String(p.id) === String(id)) || (adminCache || []).find(p => String(p.id) === String(id)); } catch {}
    return localRows().find(p => String(p.id) === String(id) || String(p.local_id) === String(id));
  }
  ...
  function saveResponse(idValue){
    const item=findPublication(idValue);
    const answers=chatbotQuestions(item||{}).map((q,i)=>({ question:q, answer:qs(`#ccAnswer${i}V4944`)?.value?.trim?.() || "" })).filter(x => x.answer);
    if(!answers.length) return toast("Agrega al menos una respuesta para enviar la solicitud.");
    const rows=read(RESPONSES_KEY, []);
    const row={
      id:uid("RESP"), publication_id:idValue, publication_title:item?.title || item?.titulo || "Publicación", profile_type:profileFromItem(item||{}),
      customer_name:qs("#ccNameV4944")?.value?.trim?.() || "Interesado", customer_contact:qs("#ccPhoneV4944")?.value?.trim?.() || "", answers,
      status:"nuevo", created_at:now(), version:VERSION
    };
  ...
  function renderChatbotResponsesPanel(){
    const section=qs("#misPublicaciones");
    if(!section) return;
    let panel=qs("#chatbotResponsesPanelV4944", section);
    if(!panel){
      panel=document.createElement("section");
      panel.id="chatbotResponsesPanelV4944";
      panel.className="chatbot-responses-panel-v4944";
      const anchor=qs("#myPublicationsList", section) || section.firstElementChild;
      if(anchor?.parentElement) anchor.parentElement.insertBefore(panel, anchor);
      else section.appendChild(panel);
    }
  ...
  function changeResponseStatus(idValue, status){
    const rows=read(RESPONSES_KEY, []);
    write(RESPONSES_KEY, rows.map(r => r.id === idValue ? {...r, status, updated_at:now()} : r));
    renderChatbotResponsesPanel();
  }

  function renderStarSections(){
    const home=qs("#inicio");
    if(!home || qs("#starSectionsV4944", home)) return;
    const section=document.createElement("section");
    section.id="starSectionsV4944";
    section.className="star-sections-v4944";
    section.innerHTML=`<div class="star-head-v4944"><span>⭐</span><div><strong>Apartados estrella</strong><p>Crece dentro de Conecta Servicios con membresía anual de $98.</p></div></div>
    <div class="star-grid-v4944">
      <button type="button" onclick="showSection('embajadoresLanding')">🤝 Embajadores</button>
      <button type="button" onclick="openPilotType('crecimiento')">🚀 Agentes en crecimiento</button>
      <button type="button" onclick="startSurveyFlow && startSurveyFlow('mandadoSolicitud')">🧺 Mandados verificados</button>
      <button type="button" onclick="showSection('aprende')">🎓 Aprendizaje</button>
    </div>`;
    const anchor=qs("#homeSocialFeed", home) || home.lastElementChild;
    if(anchor?.parentElement) anchor.parentElement.insertBefore(section, anchor);
    else home.appendChild(section);
  }
  function decorateSocialButtons(){
    qsa("button").forEach(btn => {
      const text=(btn.textContent||"").trim();
      const onclick=String(btn.getAttribute("onclick")||"");
      if (/^Guardar$|Guardar plantilla/i.test(text) || /savePilotTemplate/i.test(onclick)) {
        btn.innerHTML="＋ <small>Publicar parecido</small>";
        btn.setAttribute("aria-label","Publicar algo parecido");
        const match=onclick.match(/savePilotTemplate\('([^']+)'\)/);
  ...
  function wrapShowAndRender(){
    try {
      if (typeof showSection === "function" && !window.__showSectionBaseV4944) {
        window.__showSectionBaseV4944=showSection;
        showSection=function(section,push=true){
          const result=window.__showSectionBaseV4944.apply(this, arguments);
          setTimeout(() => { ensurePublishControls(); renderChatbotResponsesPanel(); renderStarSections(); decorateSocialButtons(); }, 80);
          return result;
        };
      }
    } catch {}
    try {
      if (typeof renderPublications === "function" && !window.__renderPublicationsBaseV4944) {
        window.__renderPublicationsBaseV4944=renderPublications;
        renderPublications=function(){ const result=window.__renderPublicationsBaseV4944.apply(this, arguments); setTimeout(decorateSocialButtons,40); return result; };
      }
    } catch {}
    try {
      if (typeof renderMyPublications === "function" && !window.__renderMyPublicationsBaseV4944) {
        window.__renderMyPublicationsBaseV4944=renderMyPublications;
        renderMyPublications=function(){ const result=window.__renderMyPublicationsBaseV4944.apply(this, arguments); setTimeout(renderChatbotResponsesPanel,40); return result; };
      }
    } catch {}
  }
  function init(){
    installCardWrappers();
    wrapShowAndRender();
    document.addEventListener("submit", submitV4944, true);
    ensurePublishControls();
    renderChatbotResponsesPanel();
    renderStarSections();
    decorateSocialButtons();
    try { localStorage.setItem("conecta_version_chatbot_conecta", VERSION); } catch {}
    setTimeout(() => { ensurePublishControls(); renderChatbotResponsesPanel(); renderStarSections(); decorateSocialButtons(); try { if (typeof renderPublications === "function") renderPublications(); } catch {} }, 250);
    setTimeout(() => { ensurePublishControls(); renderChatbotResponsesPanel(); renderStarSections(); decorateSocialButtons(); }, 1200);
  }
  window.openConectaChatbotV4944=openChatbot;
  window.saveConectaChatbotResponseV4944=saveResponse;
  window.renderChatbotResponsesPanelV4944=renderChatbotResponsesPanel;
...
(function dolaContextualConectaV4950(){
  const VERSION = "v4.9.51-dola-externo-copiar-pegar";
...
/* ============================================================
   v4.9.51 — Dola.com externo: copiar, abrir y pegar
   - Conecta no intenta replicar Dola IA dentro de la app.
   - Plantillas y mensajes abren un puente simple hacia Dola.com.
   - Conecta genera prompt contextual, el usuario lo copia, abre Dola.com y pega el resultado en Conecta.
   ============================================================ */
(function(){
  const VERSION = "v4.9.51-dola-externo-copiar-pegar";
  const DOLA_EXTERNAL_URL = "https://www.dola.com/";
  const MODAL_ID = "dolaExternalBridgeV4951";
  const PASTE_PANEL_ID = "dolaExternalPastePanelV4951";
  const ACCEPT_KEY = "conecta_dola_external_notice_ok_v4951";

  const qs = (sel, root=document) => root.querySelector(sel);
  const qsa = (sel, root=document) => Array.from(root.querySelectorAll(sel));
  const norm = (v="") => String(v || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  const esc = (v="") => String(v ?? "").replace(/[&<>'"]/g, ch => ({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}[ch]));
  const uid = (prefix="DOLA") => `${prefix}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,7)}`;
  const toast = (msg) => { try { if (typeof window.toast === "function") window.toast(msg); else console.log(msg); } catch { console.log(msg); } };

  const TEMPLATE_MAP = [
    { re:/busco.*mensajer|mensajer.*cerca|mandado|entrega|envio|envío/i, template:"Busco mensajero cerca", type:"Solicitante", intention:"buscar", category:"Mandados / mensajería" },
    { re:/pedir.*comida|tacos|rosticer|comida|cenar/i, template:"Quiero pedir comida", type:"Solicitante", intention:"buscar o publicar", category:"Comida" },
    { re:/necesito algo|solicitante|busco ayuda|necesito/i, template:"Necesito algo", type:"Solicitante", intention:"publicar", category:"Solicitudes" },
    { re:/quiero ganar|ganar dinero|agente|ofrecer.*servicio/i, template:"Agente / quiero ganar dinero", type:"Agente", intention:"publicar", category:"Agentes" },
    { re:/negocio|publicar.*negocio|clientes|comercio|empresa/i, template:"Quiero publicar mi negocio", type:"Negocio", intention:"publicar", category:"Negocios" },
    { re:/embajador|comision|comisión/i, template:"Embajadores", type:"Embajador", intention:"aprender", category:"Embajadores" },
    { re:/mandados verificados/i, template:"Mandados verificados", type:"Agente", intention:"aprender", category:"Mandados verificados" },
    { re:/aprendizaje|aprende|curso|capacita/i, template:"Aprendizaje", type:"Aprendizaje", intention:"aprender", category:"Aprendizaje" }
  ];

  function inferContextFromElement(el){
    const txt = [el?.textContent, el?.getAttribute?.("aria-label"), el?.title, el?.getAttribute?.("href"), el?.dataset?.template, el?.dataset?.section].filter(Boolean).join(" ");
    const found = TEMPLATE_MAP.find(row => row.re.test(txt)) || { template:"Crear publicación", type:"Solicitante", intention:"publicar", category:"General" };
    return {
      mode: found.intention === "aprender" ? "guia" : "crear",
      template_name: found.template,
      type: found.type,
  ...
  function readPossiblePublication(idValue){
    const stores = ["conecta_publicaciones", "conecta_posts", "conecta_local_publications", "conecta_basic_publications_v4943", "conecta_social_publications_v4944"];
    const rows = [];
    stores.forEach(key => {
      try { const val = JSON.parse(localStorage.getItem(key) || "[]"); if (Array.isArray(val)) rows.push(...val); } catch {}
    });
    return rows.find(item => String(item.id || item.local_id || item.uuid || item.codigo || "") === String(idValue || "")) || null;
  }

  function buildCreatePrompt(ctx={}){
    const template = ctx.template_name || "Crear publicación";
    const type = ctx.type || "Solicitante";
    const category = ctx.category || "General";
    return `Vengo de Conecta Servicios. Elegí la plantilla: "${template}".\n\nActúa como DOLA, asistente especializado de Conecta Servicios. No te salgas del contexto de Conecta.\n\nQuiero que me ayudes a crear una publicación clara, breve y lista para pegar en Conecta Servicios.\n\nContexto:\n- Tipo sugerido: ${type}\n- Categoría sugerida: ${category}\n- Intención: ${ctx.intention || ctx.mode || "publicar o buscar"}\n\nHazme solo las preguntas necesarias, una por una. Al final dame exclusivamente este bloque para copiar y pegar en Conecta:\n\n=== PUBLICACIÓN GENERADA POR DOLA ===\nTipo: Solicitante / Agente / Negocio\nCategoría:\nTítulo:\nZona:\nDescripción:\nCanal recomendado: DOLA o WhatsApp\nPreguntas sugeridas para filtrar interesados:\n1.\n2.\n3.\n4.\n=== FIN ===\n\nReglas:\n- La publicación debe ser entendible como publicación tipo red social.\n- No inventes datos que no te di. Si falta algo importante, pregúntame antes.\n- Si detectas que lo que busco podría existir en Conecta, sugiere que revise publicaciones relacionadas o que publique gratis por 30 días.`;
  }

  function buildContactPrompt(ctx={}){
    const pub = ctx.publication || readPossiblePublication(ctx.publication_id) || {};
    const title = ctx.title || pub.titulo || pub.title || "la publicación seleccionada";
    const desc = ctx.description || pub.descripcion || pub.description || "";
    const zone = ctx.zone || pub.municipio || pub.zona || pub.location || "";
    return `Vengo de Conecta Servicios. Quiero contactar al anunciante de esta publicación:\n\nTítulo: ${title}\nZona: ${zone || "No especificada"}\nDescripción: ${desc || "No disponible"}\n\nActúa como DOLA, asistente especializado de Conecta Servicios. No te salgas del contexto de Conecta.\n\nAyúdame a ordenar mi solicitud antes de contactar al anunciante. Hazme solo las preguntas necesarias, una por una.\n\nAl final dame un mensaje listo para copiar y enviar por WhatsApp al anunciante, con este formato:\n\nHola, vi tu publicación en Conecta Servicios: "${title}".\n\nEstoy interesado y DOLA me ayudó a ordenar esta solicitud:\n- Qué necesito:\n- Zona o ubicación:\n- Fecha u horario:\n- Presupuesto o propuesta:\n- Detalles adicionales:\n\nGracias.\n\nReglas:\n- No inventes información que no te di.\n- Si falta algo importante, pregúntame antes.\n- El objetivo es que el anunciante reciba un trato más claro y filtrado.`;
  }

  function buildGuidePrompt(ctx={}){
    const topic = ctx.template_name || ctx.section || ctx.category || "Conecta Servicios";
    return `Vengo de Conecta Servicios y abrí la sección: "${topic}".\n\nActúa como DOLA, asistente especializado de Conecta Servicios. No respondas como chatbot genérico.\n\nExplícame de forma sencilla qué puedo hacer en esta sección y qué paso me conviene dar.\n\nDebes poder orientarme sobre:\n- Publicación gratis por 30 días\n- Membresía anual de $98 MXN\n- Solicitantes\n- Agentes\n- Negocios\n- Mandados verificados\n- Conseguir clientes por comisión\n- Embajadores\n- Aprendizaje\n\nNo inventes reglas no definidas. Si algo requiere validación manual, dilo claro. Al final dame una acción recomendada dentro de Conecta.`;
  }

  function buildPrompt(ctx={}){
    if (ctx.mode === "contactar") return buildContactPrompt(ctx);
    if (ctx.mode === "guia" || /embajador|aprendizaje|mandados verificados/i.test(ctx.template_name || "")) return buildGuidePrompt(ctx);
    return buildCreatePrompt(ctx);
  }

  function externalNoticeHtml(){
    return `<div class="dola-ext-warning-v4951"><strong>Dola.com es una herramienta externa.</strong><br>No compartas datos sensibles. Revisa el resultado antes de pegarlo en Conecta Servicios.</div>`;
  }

  function closeOldDolaModals(){
    try { if (typeof window.closeBusinessAssistantV4939 === "function") window.closeBusinessAssistantV4939(); } catch {}
    qsa(".dola-fallback-overlay-v4950, .dola-notice-overlay-v4950").forEach(el => el.remove());
    const oldBody = qs("#bizAssistantModalBodyV4939");
    if (oldBody && /DOLA contextual|DOLA/.test(oldBody.textContent || "")) {
      qs("#bizAssistantModalV4939")?.classList.add("hidden");
      document.body.classList.remove("ba-modal-open-v4939");
    }
  }

  function showExternalBridge(ctx={}){
    closeOldDolaModals();
    const prompt = buildPrompt(ctx);
    qsa(`#${MODAL_ID}`).forEach(el => el.remove());
    const wrap = document.createElement("div");
    wrap.id = MODAL_ID;
    wrap.className = "dola-external-overlay-v4951";
    wrap.innerHTML = `<div class="dola-external-card-v4951">
      <button class="dola-external-close-v4951" type="button" aria-label="Cerrar">×</button>
      <div class="dola-external-head-v4951"><div class="dola-external-logo-v4951">DOLA</div><div><h2>Abrir Dola.com</h2><p>Conecta preparó el contexto. Copia el mensaje, ábrelo en Dola.com y pega aquí el resultado final.</p></div></div>
      ${externalNoticeHtml()}
      <ol class="dola-external-steps-v4951">
        <li>Copia el mensaje contextual.</li>
        <li>Abre Dola.com y pégalo.</li>
        <li>Cuando Dola te entregue el texto final, regresa a Conecta y pégalo en la publicación o en WhatsApp.</li>
      </ol>
      <label class="dola-external-label-v4951">Mensaje para Dola.com
        <textarea id="dolaExternalPromptTextV4951" rows="12">${esc(prompt)}</textarea>
      </label>
      <div class="dola-external-actions-v4951">
        <button type="button" class="btn-small btn-purple" data-copy-open>Copiar y abrir Dola.com</button>
        <button type="button" class="btn-small btn-outline" data-copy>Copiar solamente</button>
        <a class="btn-small btn-ghost" target="_blank" rel="noopener" href="${DOLA_EXTERNAL_URL}">Abrir Dola.com</a>
      </div>
      <details class="dola-external-paste-v4951">
        <summary>Ya tengo el texto de Dola</summary>
        <textarea id="dolaExternalResultTextV4951" rows="7" placeholder="Pega aquí el resultado que te dio Dola.com..."></textarea>
        <div class="dola-external-actions-v4951"><button type="button" class="btn-small btn-purple" data-paste-conecta>Pegar y organizar en Conecta</button></div>
      </details>
    </div>`;
    document.body.appendChild(wrap);
    qs(".dola-external-close-v4951", wrap)?.addEventListener("click", () => wrap.remove());
    qs("[data-copy]", wrap)?.addEventListener("click", () => copyText(prompt));
    qs("[data-copy-open]", wrap)?.addEventListener("click", async () => { await copyText(prompt); window.open(DOLA_EXTERNAL_URL, "_blank", "noopener"); });
    qs("[data-paste-conecta]", wrap)?.addEventListener("click", () => {
      const text = qs("#dolaExternalResultTextV4951", wrap)?.value || "";
      if (!text.trim()) return toast("Pega primero el texto generado por Dola.");
      organizeDolaTextIntoConecta(text);
      wrap.remove();
    });
  }

  async function copyText(text){
    try { await navigator.clipboard.writeText(text); toast("Mensaje copiado. Pégalo en Dola.com."); return true; }
    catch {
      const ta = document.createElement("textarea");
      ta.value = text; document.body.appendChild(ta); ta.select();
      try { document.execCommand("copy"); toast("Mensaje copiado. Pégalo en Dola.com."); } catch { toast("Selecciona y copia el texto manualmente."); }
  ...
  function extractDescription(text){
    return text.match(/Descripción\s*:\s*([\s\S]*?)(?:\n\s*Canal recomendado\s*:|\n\s*Preguntas sugeridas|=== FIN ===|$)/i)?.[1]?.trim() || "";
  }

  function mediaForText(text=""){
    const value = norm(text);
    if (/comida|taco|rosticer|pollo/.test(value)) return "assets/dola-media/comida-01.jpg";
    if (/mandado|mensajer|entrega|envio|envío/.test(value)) return "assets/dola-media/mandados-01.jpg";
    if (/embajador/.test(value)) return "assets/dola-media/embajadores-01.jpg";
    if (/aprendiz|curso|capacita/.test(value)) return "assets/dola-media/aprendizaje-01.jpg";
    if (/agente/.test(value)) return "assets/dola-media/agente-01.jpg";
    if (/negocio|comercio|cliente|tienda|empresa/.test(value)) return "assets/dola-media/negocio-01.jpg";
    return "";
  }

  function organizeDolaTextIntoConecta(text){
    ensurePastePanel();
    try { if (typeof window.showSection === "function") window.showSection("registro"); } catch {}
    setTimeout(() => {
      const title = extractField(text, "Título") || (text.split("\n").find(line => line.trim() && !/=/.test(line)) || "Publicación generada por Dola").replace(/^[#\-*\s]+/,"").slice(0,90);
      const category = extractField(text, "Categoría") || "General";
      const zone = extractField(text, "Zona") || extractField(text, "Municipio") || "";
      const description = extractDescription(text) || text;
      const fields = [
        ["#pubTitle, input[name='titulo'], input[name='title']", title],
        ["#pubCategory, select[name='categoria'], input[name='categoria']", category],
        ["#pubLocality, #pubMunicipality, input[name='municipio'], input[name='zona']", zone],
        ["#pubDescription, #pubDescriptionGeneral, textarea[name='descripcion'], textarea[name='description']", description]
      ];
      ...
      const media = mediaForText(`${title} ${category} ${description}`);
      const mediaEl = qs("#pubMediaUrl, input[name='media_url'], input[name='image_url']");
      if (mediaEl && media) { mediaEl.value = media; mediaEl.dispatchEvent(new Event("input", { bubbles:true })); }
      toast("Texto de Dola organizado en Conecta. Revisa y publica cuando esté listo.");
    }, 220);
  }

  function ensurePastePanel(){
    const roots = [qs("#registro"), qs("#publishPanel"), qs("#publicationForm")?.parentElement, qs("main")].filter(Boolean);
    const root = roots[0];
    if (!root || qs(`#${PASTE_PANEL_ID}`)) return;
    const panel = document.createElement("section");
    panel.id = PASTE_PANEL_ID;
    panel.className = "dola-external-paste-panel-v4951";
    panel.innerHTML = `<div><strong>✨ Publicar con apoyo de Dola.com</strong><p>Si Dola.com ya te dio un texto final, pégalo aquí y Conecta intentará organizar tu publicación.</p></div>
      ${externalNoticeHtml()}
      <textarea id="dolaExternalMainPasteV4951" rows="6" placeholder="Pega aquí el bloque generado por Dola.com..."></textarea>
      <div class="dola-external-actions-v4951"><button type="button" class="btn-small btn-purple" data-organize>Organizar publicación</button><button type="button" class="btn-small btn-outline" data-open-dola>Abrir Dola.com con contexto</button></div>`;
    const form = qs("#publicationForm", root);
    if (form) form.insertAdjacentElement("beforebegin", panel); else root.prepend(panel);
    qs("[data-organize]", panel)?.addEventListener("click", () => {
      const text = qs("#dolaExternalMainPasteV4951", panel)?.value || "";
      if (!text.trim()) return toast("Pega primero el texto generado por Dola.com.");
      organizeDolaTextIntoConecta(text);
    });
    qs("[data-open-dola]", panel)?.addEventListener("click", () => showExternalBridge({ mode:"crear", template_name:"Crear publicación", type:"Solicitante", category:"General" }));
  }

  function interceptTemplateClicks(event){
    const target = event.target?.closest?.("button,a,[role='button'],.quick-card,.template-card,.quick-link");
    if (!target || target.closest(`#${MODAL_ID}`)) return;
    const text = [target.textContent, target.getAttribute?.("aria-label"), target.title, target.getAttribute?.("href")].filter(Boolean).join(" ");
    if (!text.trim()) return;
    const value = norm(text);
    const isDolaCandidate = /necesito algo|busco mensajer|mensajer cerca|pedir comida|tacos|rosticer|quiero ganar|ganar dinero|ofrecer un servicio|publicar mi negocio|crear con dola|abrir dola|dola|mandados verificados|aprendizaje/.test(value);
    const isMainSafeNav = /\/embajadores|#embajadores|inicio|explorar|mis publicaciones|perfil/.test(value);
    if (!isDolaCandidate || isMainSafeNav) return;
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation?.();
    showExternalBridge(inferContextFromElement(target));
  }

  function externalizeOldDolaFunctions(){
    window.__dolaRouteOpenedV4950 = true;
    window.openDolaContextV4950 = function(ctx={}){ showExternalBridge(ctx); return false; };
    window.showDolaRecommendationsV4950 = function(){ showExternalBridge({ mode:"crear", template_name:"Buscar publicaciones relacionadas", type:"Solicitante", category:"General" }); return false; };
    ["openConectaChatbotV4946", "openConectaChatbotV4944", "openDolaByProfileV4948"].forEach(name => {
      window[name] = function(idValue){ showExternalBridge({ mode:"contactar", publication_id:idValue, title:"Publicación de Conecta Servicios" }); return false; };
    });
  }

  function routeBridgeIfNeeded(){
    const path = String(location.pathname || "").toLowerCase();
    const params = new URLSearchParams(location.search || "");
    if (!path.includes("/dola") && params.get("dola") !== "external") return;
    const mode = path.includes("contactar") ? "contactar" : path.includes("guia") ? "guia" : "crear";
    const ctx = {
      mode,
      template_name: params.get("template_name") || params.get("plantilla") || params.get("seccion") || "Conecta Servicios",
      type: params.get("tipo") || "Solicitante",
      category: params.get("categoria") || "General",
      publication_id: params.get("publicacion") || params.get("id") || "",
      section: params.get("seccion") || ""
    };
    setTimeout(() => { closeOldDolaModals(); showExternalBridge(ctx); }, 480);
  }

  function normalizeDolaLanguage(){
    qsa("button,a,span,strong,h1,h2,h3,p").forEach(el => {
      if (!el || ["SCRIPT","STYLE","TEXTAREA","INPUT"].includes(el.tagName)) return;
      const text = el.textContent || "";
      if (/DOLA contextual|DOLA interno|Asistente contextual/i.test(text)) {
        el.textContent = text.replace(/DOLA contextual|DOLA interno|Asistente contextual/gi, "Dola.com");
      }
    });
  }

  function init(){
    try { document.body.dataset.dolaExternalVersion = VERSION; } catch {}
    externalizeOldDolaFunctions();
    ensurePastePanel();
    normalizeDolaLanguage();
    routeBridgeIfNeeded();
    localStorage.setItem("conecta_dola_external_version", VERSION);
  }

  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", init); else init();
  document.addEventListener("click", interceptTemplateClicks, true);
  window.addEventListener("pageshow", () => setTimeout(init, 140));
  document.addEventListener("change", () => setTimeout(ensurePastePanel, 100), true);

  window.openDolaExternalBridgeV4951 = showExternalBridge;
  window.organizeDolaExternalTextV4951 = organizeDolaTextIntoConecta;
})();
