/* Conecta Control Friendly v2
   Módulo visual aislado para /control.
   Corrige lectura de cantidades: "Vendí 3 pays de limón" = 3 x $25.
   No toca Supabase, publicaciones, mensajes ni geolocalización.
*/
(() => {
  'use strict';
  ...
  const STYLE_ID = 'cc-friendly-v2-styles';
  ...
  function normalize(value=''){
    return String(value || '')
      ...
      .replace(/[\u0300-\u036f]/g,'')
      .replace(/[“”"']/g,'')
      .replace(/\b(vendí|vendí)\b/g,'vendi')
      .replace(/\bpays\b/g,'pay')
      .replace(/\spay\s+de\s+/g,' pay de ')
      .replace(/\s+/g,' ')
      .trim();
  }
  ...
  function normalizeControls(controls){
    ...
    if(!set.size) ['VENTA','GASTO','INVENTARIO','PRODUCCION','MERMA','PEDIDO','COBRO','DEUDA','EMPLEADO'].forEach(key => set.add(key));
  ...
  function getConfig(){
    ...
    if(current){
      const clean = {...current};
      clean.products = (Array.isArray(clean.products) && clean.products.length ? clean.products : DEMO_PRODUCTS).map(productFromAny).filter(Boolean);
      clean.controls = normalizeControls(clean.controls || []);
      clean.customControls = Array.isArray(clean.customControls) ? clean.customControls : [];
      return clean;
    }
    ...
    if(legacy && typeof legacy === 'object'){
      const migrated = {
        ...
        products: Array.isArray(legacy.productosServicios) && legacy.productosServicios.length
          ? legacy.productosServicios.map(productFromAny).filter(Boolean)
          : DEMO_PRODUCTS,
  ...
  function saveConfig(config){
    const clean = {
      ...
      products: (Array.isArray(config.products) && config.products.length ? config.products : DEMO_PRODUCTS).map(productFromAny).filter(Boolean),
  ...
  function quantityWords(){
    return {un:1, uno:1, una:1, dos:2, tres:3, cuatro:4, cinco:5, seis:6, siete:7, ocho:8, nueve:9, diez:10, once:11, doce:12, trece:13, catorce:14, quince:15, dieciseis:16, diecisiete:17, dieciocho:18, diecinueve:19, veinte:20, veintiuno:21, veintidos:22, veintitres:23, treinta:30, media:0.5, medio:0.5};
  }

  function quantityPattern(){
    return ['[0-9]+(?:[\.,][0-9]+)?', ...Object.keys(quantityWords()).sort((a,b)=>b.length-a.length)].join('|');
  }
  ...
  function singularPluralVariants(alias=''){
    const base = normalize(alias);
    const variants = new Set([base]);
    if(base.includes('pay')) variants.add(base.replace(/\bpay\b/g,'pays'));
    if(base.includes('pays')) variants.add(base.replace(/\bpays\b/g,'pay'));
    if(base.endsWith('a')) variants.add(`${base}s`);
    if(base.endsWith('o')) variants.add(`${base}s`);
    return [...variants].filter(Boolean);
  }

  function aliasList(product){
    const base = new Set([product.name, ...(product.aliases || [])].map(normalize).filter(Boolean));
    const stripped = normalize(product.name).replace(/\b(de|del|la|el|los|las|con)\b/g,' ').replace(/\s+/g,' ').trim();
    if(stripped) base.add(stripped);
    normalize(product.name).split(/\s+/).filter(word => word.length > 3 && !['para','como','leche','crema'].includes(word)).forEach(word => base.add(word));
    const all = new Set();
    base.forEach(alias => singularPluralVariants(alias).forEach(v => all.add(v)));
  ...
  function extractProducts(message, config){
    ...
    const items = [];
    const products = (config.products && config.products.length ? config.products : DEMO_PRODUCTS).map(productFromAny).filter(Boolean);
    ...
    products.forEach(product => {
      ...
      while((match = beforeRx.exec(text))){
        ...
        matches.push({cantidad, index:match.index, raw:match[0]});
        if(match.index === beforeRx.lastIndex) beforeRx.lastIndex++;
      }

      // Caso dictado: "limón 3" o "fresas 2". Solo si no hubo "3 limón".
      if(!matches.length){
      ...
      if(matches.length){
        matches.forEach(hit => {
          ...
          items.push({name:product.name, qty:hit.cantidad, price, total:hit.cantidad * price});
        });
        return;
      }

      // Último recurso: producto mencionado sin cantidad. Ahí sí asumimos 1.
      const mentioned = new RegExp(`\\b(?:${pattern})(?:s)?\\b`, 'i').test(text);
      if(mentioned){
        const price = Number(product.price || 0);
  ...
  function extractMoney(message){
    ...
    const explicit = original.match(/\$\s*([0-9]+(?:[\.,][0-9]+)?)/);
    if(explicit) return Number(explicit[1].replace(',','.')) || 0;
    const text = normalize(original);
    const contextual = text.match(/(?:fueron|fue|total|costo|costaron|pague|pago|gaste|gasto|compre|cobre|cobro|deben|sueldo|apoyo)\s+([0-9]+(?:[\.,][0-9]+)?)/);
    if(contextual) return Number(contextual[1].replace(',','.')) || 0;
    const all = text.match(/[0-9]+(?:[\.,][0-9]+)?/g) || [];
  ...
  function detectIntent(message, config){
    ...
    if(/\b(empleado|empleada|empleados|ayudante|ayuda|sueldo|salario|jornal|nomina|mano de obra)\b/.test(text)) return 'EMPLEADO';
    ...
    if(/\b(inventario|conte|conteo|contado|me quedan|quedan|existencia)\b/.test(text)) return 'INVENTARIO';
  ...
  function recordLabel(type, config){
    if(String(type).startsWith('CUSTOM:')){
      ...
      return (config.customControls || []).find(item => item.id === id)?.label || 'Otro';
    }
  ...
  function parseRecord(message, config){
    ...
    if(type === 'VENTA'){
      ...
      if(!total){ status = 'pending'; response = 'Falta producto o monto. Ej: Vendí 3 pay de limón.'; }
      else response = `Venta guardada: ${money(total)}${items.length ? ` (${productLine(items)})` : ''}.`;
    }else if(type === 'GASTO'){
      total = amount;
      if(!total){ status = 'pending'; response = 'Falta cuánto gastaste. Ej: Compré leche $80.'; }
      else response = `Compra/gasto guardado: ${money(total)}.`;
    }else if(type === 'EMPLEADO'){
      total = amount;
      if(!total){ status = 'pending'; response = 'Falta cuánto pagaste. Ej: Pagué a María $300.'; }
      else response = `Pago a empleado guardado: ${money(total)}.`;
    }else if(type === 'MERMA'){
      total = items.reduce((sum,item)=>sum + Number(item.total || 0), 0) || amount;
      if(!total){ status = 'pending'; response = 'Falta producto o valor. Ej: Merma 2 arroz.'; }
      else response = `Merma guardada: ${money(total)}${items.length ? ` (${productLine(items)})` : ''}.`;
    }else if(type === 'PEDIDO'){
      total = items.reduce((sum,item)=>sum + Number(item.total || 0), 0) || amount;
      response = `Pedido guardado${total ? `: ${money(total)}` : ''}${items.length ? ` (${productLine(items)})` : ''}.`;
    }else if(type === 'COBRO'){
      total = amount;
      if(!total){ status = 'pending'; response = 'Falta cuánto cobraste.'; }
      else response = `Cobro guardado: ${money(total)}.`;
    }else if(type === 'DEUDA'){
      total = amount;
      if(!total){ status = 'pending'; response = 'Falta cuánto te deben.'; }
      else response = `Fiado/deuda guardado: ${money(total)}.`;
    }else if(type === 'INVENTARIO' || type === 'PRODUCCION'){
      response = `${recordLabel(type, config)} guardado${items.length ? `: ${productLine(items)}` : ''}.`;
    }else if(custom){
      total = amount;
      response = `${custom.label} guardado${total ? `: ${money(total)}` : ''}.`;
    }else{
      response = 'Nota guardada.';
    }

    return {id:uid('ccr'), date:new Date().toISOString(), day:todayKey(), type, label:recordLabel(type, config), message:String(message || '').trim(), items, total, status, response, customFlow:custom?.flow || ''};
  }
  ...
  function summary(config){
    ...
    return {records, today, ventas, compras, empleados, mermas, cobros, deudas, customIn, customOut, utilidad:ventas + customIn - compras - empleados - mermas - customOut, vendidos:Object.values(sold).reduce((a,b)=>a+b,0), top:top ? `${top[0]} (${top[1]})` : 'Sin ventas'};
  }
  ...
  function injectStyles(){
    ...
    style.textContent = `
      ...
      .cc-friendly *{box-sizing:border-box}.cc-top{display:flex;gap:12px;align-items:center;justify-content:space-between;margin-bottom:12px}.cc-back{min-height:44px;border-radius:999px;padding:0 14px;background:#fff;border:1px solid #e5e7eb;color:#5b2eea;font-weight:950;box-shadow:0 8px 20px rgba(15,23,42,.08)}
      ...
      .cc-panel{background:#fff;border:1px solid #e8edf3;border-radius:28px;padding:14px;margin:12px 0;box-shadow:0 16px 34px rgba(15,23,42,.08)}.cc-panel h2{font-size:19px;margin:0 0 8px;letter-spacing:-.03em}.cc-panel p{margin:0 0 10px;color:#64748b;font-weight:800;line-height:1.34}.cc-big-btn,.cc-soft-btn{width:100%;min-height:58px;border-radius:22px;border:0;font-weight:1000;font-size:18px;margin-top:10px}.cc-big-btn{background:linear-gradient(135deg,#16a34a,#059669);color:#fff;box-shadow:0 14px 28px rgba(22,163,74,.26)}.cc-soft-btn{background:#f1f5f9;color:#334155;border:1px solid #e2e8f0}.cc-soft-btn.danger{background:#fff1f2;color:#be123c;border-color:#fecdd3}
      ...
      .cc-input{width:100%;min-height:112px;border-radius:24px;border:2px solid #dbe4ef;background:#fff;padding:14px;font-size:18px;font-weight:800;outline:none;resize:vertical}.cc-input:focus{border-color:#16a34a;box-shadow:0 0 0 4px rgba(22,163,74,.16)}.cc-chip-row{display:flex;gap:8px;overflow:auto;padding:8px 2px 2px;-webkit-overflow-scrolling:touch}.cc-chip{flex:0 0 auto;border-radius:999px;background:#eef2ff;color:#3730a3;border:1px solid #c7d2fe;padding:10px 12px;font-weight:950}
      .cc-history{display:grid;gap:9px;max-height:420px;overflow:auto;padding-right:2px}.cc-record{border-radius:20px;border:1px solid #e2e8f0;background:#f8fafc;padding:11px}.cc-record b{display:flex;gap:8px;align-items:center;font-size:15px}.cc-record p{margin:6px 0 0;color:#334155}.cc-record small{display:block;margin-top:5px;color:#64748b;font-weight:850}.cc-form-grid{display:grid;gap:10px}.cc-field{width:100%;min-height:52px;border:1px solid #cbd5e1;border-radius:18px;padding:12px;font-size:16px;font-weight:850}.cc-products{min-height:116px}.cc-empty{padding:18px;border-radius:22px;background:#f8fafc;border:1px dashed #cbd5e1;text-align:center;color:#64748b;font-weight:900}
  ...
  function renderWelcome(target){
    target.innerHTML = `<section class="cc-friendly" data-cc-friendly="1"><div class="cc-top"><button class="cc-back" data-nav="/">← Inicio</button></div><div class="cc-hero"><p class="cc-kicker">Libreta inteligente</p><h1>Conecta Control</h1><p>Tu negocio claro con botones grandes, colores y pocas palabras.</p></div><div class="cc-panel"><h2>¿Qué hacemos?</h2><div class="cc-grid"><button class="cc-tile active" data-cc-demo><span class="ico">🍰</span><strong>Postres Fer</strong><small>Probar demo</small></button><button class="cc-tile" data-cc-setup><span class="ico">🧩</span><strong>Mi negocio</strong><small>Crear tablero</small></button></div></div><div class="cc-panel"><h2>Botones posibles</h2><div class="cc-grid">${BASE_CONTROLS.slice(0,8).map(c => `<div class="cc-tile"><span class="ico">${c.icon}</span><strong>${esc(c.label)}</strong><small>${esc(c.short)}</small></div>`).join('')}<div class="cc-tile"><span class="ico">➕</span><strong>Otro</strong><small>Agregar lo tuyo</small></div></div></div></section>`;
  }
  ...
  function renderSetup(target){
    const draft = getConfig() || {name:'', type:'', products:DEMO_PRODUCTS, controls:['VENTA','GASTO','INVENTARIO','PRODUCCION','MERMA','PEDIDO','COBRO','DEUDA','EMPLEADO'], customControls:[]};
    const productText = (draft.products || DEMO_PRODUCTS).map(p => `${p.name}${p.price ? ` ${p.price}` : ''}`).join('\n');
    target.innerHTML = `<section class="cc-friendly" data-cc-friendly="1" data-cc-mode="setup"><div class="cc-top"><button class="cc-back" data-cc-cancel>← Salir</button></div><div class="cc-hero"><p class="cc-kicker">Configurar</p><h1>Mi tablero</h1><p>Elige solo lo que usas. Después puedes agregar más.</p></div><div class="cc-panel cc-form-grid"><input class="cc-field" id="ccName" placeholder="Nombre del negocio" value="${esc(draft.name || '')}"><input class="cc-field" id="ccType" placeholder="Tipo: postres, comida, servicios..." value="${esc(draft.type || '')}"><textarea class="cc-field cc-products" id="ccProducts" placeholder="Productos y precios&#10;Pay de limón 25&#10;Fresas con crema 35">${esc(productText)}</textarea></div><div class="cc-panel"><h2>Mis botones</h2><div class="cc-grid">${BASE_CONTROLS.map(c => `<button class="cc-tile ${draft.controls?.includes(c.key) ? 'active' : ''}" data-cc-toggle="${c.key}"><span class="ico">${c.icon}</span><strong>${esc(c.label)}</strong><small>${esc(c.short)}</small></button>`).join('')}<button class="cc-tile active" data-cc-add-custom><span class="ico">➕</span><strong>Otro</strong><small>Crear botón</small></button></div></div><button class="cc-big-btn" data-cc-save-setup>Guardar tablero</button></section>`;
  }

  function createDemo(){
    saveConfig({id:'postres_fer', name:'Postres Fer', type:'Postres', products:DEMO_PRODUCTS, controls:['VENTA','GASTO','INVENTARIO','PRODUCCION','MERMA','PEDIDO','COBRO','DEUDA','EMPLEADO'], customControls:[{id:'entregas', icon:'🛵', label:'Entregas', flow:'salida'}]});
    renderCurrent();
  }
  ...
  function renderDashboard(target, config){
    ...
    const examples = ['Vendí 3 pays de limón','Vendí 3 pay de limón','Compré leche $80','Pagué a María $300','Merma 2 arroz'];
    target.innerHTML = `<section class="cc-friendly" data-cc-friendly="1"><div class="cc-top"><button class="cc-back" data-nav="/">← Inicio</button><button class="cc-back" data-cc-setup>⚙️ Editar</button></div><div class="cc-hero"><p class="cc-kicker">${esc(config.type || 'Negocio')}</p><h1>${esc(config.name || 'Mi negocio')}</h1><p>¿Qué pasó hoy?</p></div><div class="cc-stats"><div class="cc-stat good"><span>Ventas</span><strong>${money(stats.ventas)}</strong></div><div class="cc-stat warn"><span>Compras</span><strong>${money(stats.compras)}</strong></div><div class="cc-stat warn"><span>Empleados</span><strong>${money(stats.empleados)}</strong></div><div class="cc-stat bad"><span>Mermas</span><strong>${money(stats.mermas)}</strong></div><div class="cc-stat ${stats.utilidad >= 0 ? 'good' : 'bad'}"><span>Me quedó</span><strong>${money(stats.utilidad)}</strong></div><div class="cc-stat"><span>Más vendido</span><strong>${esc(stats.top)}</strong></div></div><div class="cc-panel"><h2>Botones rápidos</h2><div class="cc-grid">${controls.map(c => `<button class="cc-tile" data-cc-prefill="${c.key}"><span class="ico">${c.icon}</span><strong>${esc(c.label)}</strong><small>${esc(c.short)}</small></button>`).join('')}${custom.map(c => `<button class="cc-tile" data-cc-prefill-custom="${esc(c.id)}"><span class="ico">${esc(c.icon || '⭐')}</span><strong>${esc(c.label)}</strong><small>Mi botón</small></button>`).join('')}<button class="cc-tile active" data-cc-add-custom><span class="ico">➕</span><strong>Otro</strong><small>Agregar</small></button></div></div><div class="cc-panel"><h2>Escribir</h2><textarea class="cc-input" id="ccMessage" placeholder="Ej: Vendí 3 pays de limón"></textarea><button class="cc-big-btn" data-cc-process>Guardar</button><div class="cc-chip-row">${examples.map(e => `<button class="cc-chip" data-cc-example="${esc(e)}">${esc(e)}</button>`).join('')}</div></div><div class="cc-panel"><h2>Hoy</h2><div class="cc-history">${stats.records.slice().reverse().slice(0,24).map(recordMarkup).join('') || '<div class="cc-empty">Aún no hay movimientos.</div>'}</div><button class="cc-soft-btn danger" data-cc-clear>Limpiar prueba</button></div></section>`;
  }
  ...
  function activeTarget(){
    return document.querySelector('.control-page, .cc-friendly');
  }
  ...
  function addCustomControl(){
    ...
    const label = prompt('¿Cómo quieres llamar tu botón? Ej: Renta, Entregas, Gasolina');
  ...
  function prefill(type){
    ...
    if(!input) return;
    const map = {VENTA:'Vendí ', GASTO:'Compré ', INVENTARIO:'Inventario ', PRODUCCION:'Hice ', MERMA:'Merma ', PEDIDO:'Pedido para mañana: ', COBRO:'Cobré ', DEUDA:'Me deben ', EMPLEADO:'Pagué a '};
  ...
  function processMessage(){
    ...
    saveRecord(parseRecord(message, config));
  ...
  function saveSetupFromDom(){
    ...
    const config = saveConfig({...existing, name:document.getElementById('ccName')?.value || existing.name || 'Mi negocio', type:document.getElementById('ccType')?.value || existing.type || 'Negocio', products:parseProductsText(document.getElementById('ccProducts')?.value || ''), controls, customControls:existing.customControls || []});
    renderDashboard(activeTarget(), config);
  }

  // Captura en window para evitar que la versión anterior procese dos veces el click si aún está en caché.
  ...
  const observer = new MutationObserver(() => {
    ...
    if(target && !target.querySelector('[data-cc-friendly="1"]')) setTimeout(renderCurrent, 0);
  });
